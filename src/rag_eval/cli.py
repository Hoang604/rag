...
@app.command()
def download(
    dataset: Annotated[str, typer.Option("--dataset", "-d", help="Dataset name: cuad | qasper | scifact | beir_fiqa | all")],
    output_dir: Annotated[str, typer.Option("--output-dir", "-o", help="Root storage directory")] = "./data",
) -> None:
    ...
    if target == "all":
        ...
        console.print(f"[bold red]Error:[/bold red] Unknown dataset '{dataset}'. Choose from: {list(downloaders.keys())} or 'all'")
        raise typer.Exit(code=1)

    for name in targets_to_run:
        label, dl_fn = downloaders[name]
        console.print(f"[cyan]Downloading and normalizing [bold]{label}[/bold]...[/cyan]")
        bmark = dl_fn()
        doc_count = len(bmark.documents)
        query_count = len(bmark.queries)
        gt_count = len(bmark.ground_truths)
        dest = str(out_path / name)
        console.print(f"[green]✔ Loaded {doc_count} documents, {query_count} queries, {gt_count} qrels -> {dest}[/green]")
...
def baseline(
    ...
    candidate_pool_size: Annotated[int, typer.Option("--candidate-pool-size", help="Number of BM25 candidate passages to re-score")] = 150,
    chunk_size: Annotated[int, typer.Option("--chunk-size", help="Character size per document chunk")] = 512,
    chunk_overlap: Annotated[int, typer.Option("--chunk-overlap", help="Character overlap between chunks")] = 64,
    max_queries: Annotated[int | None, typer.Option("--max-queries", "-n", help="Optional limit on number of queries to evaluate")] = None,
    seed: Annotated[int | None, typer.Option("--seed", help="Optional random seed for representative query sampling")] = None,
) -> None:
    ...
    if norm_mode not in ("hybrid", "bm25", "dense"):
        console.print(f"[bold red]Error:[/bold red] Unknown mode '{mode}'. Choose from: hybrid | bm25 | dense")
        raise typer.Exit(code=1)
    ...
    if target not in loaders:
        console.print(f"[bold red]Error:[/bold red] Unknown dataset '{dataset}'. Choose from: {list(loaders.keys())}")
        raise typer.Exit(code=1)
    ...
    try:
        ...
        console.print(f"[bold red]Error:[/bold red] Dataset '{target}' not found in '{data_dir}'. Run `rag-eval download -d {target}` first. ({err})")
        raise typer.Exit(code=1) from err

    total_available = len(bmark.queries)
    eval_target_count = min(max_queries, total_available) if max_queries is not None else total_available
    sample_mode = f" (seeded sample: {seed})" if seed is not None else ""
    console.print(f"[cyan]Running [bold]{norm_mode.upper()}[/bold] retrieval on [bold]{label}[/bold] ({len(bmark.documents)} docs, {eval_target_count}/{total_available} queries{sample_mode})...[/cyan]")
    ...
    console.print(f"[green]✔ Generated {len(predictions)} baseline predictions -> {out_file}[/green]")
...
def evaluate(
    ...
    if not pred_path.is_file():
        console.print(f"[bold red]Error:[/bold red] Predictions file not found: {pred_path}")
        raise typer.Exit(code=1)
    ...
    console.print(table)
    console.print(f"[dim]Total Dataset Queries: {report.total_queries} | Evaluated Queries: {report.evaluated_queries}[/dim]\n")
