-- ----------------------------------------------------------------------------
-- ADMIT PHRASE MATCHES TO THE SPARSE POOL
--
-- The phrase bonus was applied after the pool was cut. "Xe máy vượt đèn đỏ"
-- ranks its answer 821st by ts_rank -- the words "vượt" and "đỏ" are common and
-- the statute says "không chấp hành hiệu lệnh của đèn tín hiệu giao thông" --
-- so it never reached the 240-row shortlist where the bonus is paid, and rank 1
-- went to a clause about red warning markers on protruding cargo.
--
-- A literal hit on a statutory phrase is the strongest signal available, so it
-- now buys entry to the pool rather than only a bonus once inside.
-- ----------------------------------------------------------------------------
