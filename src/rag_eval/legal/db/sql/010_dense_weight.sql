-- ----------------------------------------------------------------------------
-- WEIGHT THE DENSE SIDE BY HOW MUCH THE QUERY CAN BE TRUSTED TO IT
--
-- Most people type Vietnamese without tone marks. The text search config folds
-- diacritics, so the sparse ranker is unaffected -- measured, "xe máy vượt đèn
-- đỏ" and "xe may vuot den do" produce the identical tsquery. The embedding is
-- not: the same answer sits at dense rank 20 accented and outside the whole
-- candidate set unaccented, while staying sparse rank 1 both ways.
--
-- So the fusion stops treating the two sides as equally informative. When the
-- caller reports that the query carries no diacritics, the dense contribution
-- is discounted rather than discarded -- it is degraded evidence, not absent
-- evidence, and a query that is unaccented only because it is a sign code
-- ("P.118") still deserves whatever the vector found.
-- ----------------------------------------------------------------------------
