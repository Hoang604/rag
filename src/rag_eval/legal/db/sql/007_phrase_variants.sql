-- ----------------------------------------------------------------------------
-- PHRASE BONUS ON STATUTORY PHRASES
--
-- The phrase bonus was dead code. phraseto_tsquery over a whole question
-- ("xe may vuot den do phat bao nhieu") demands those eight syllables adjacent
-- in that order, which no statute contains -- zero matches out of 7,112 chunks
-- for every query measured. Query expansion made it worse by lengthening the
-- phrase.
--
-- The phrases that do occur are the statutory ones the lexicon maps onto, so
-- the caller passes those and the bonus is the OR over them. "Xe máy vượt đèn
-- đỏ" now scores a literal hit on "Không chấp hành hiệu lệnh của đèn tín hiệu
-- giao thông" instead of losing rank 1 to a clause that merely contains the
-- syllables "vượt" and "đỏ".
-- ----------------------------------------------------------------------------

DROP FUNCTION IF EXISTS hybrid_search(TEXT, VECTOR(384), DATE, INT, INT, TEXT, TEXT);
