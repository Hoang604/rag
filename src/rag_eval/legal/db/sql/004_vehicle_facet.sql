...
-- VEHICLE-CLASS FACET
--
-- Nghị định 168/2024 states the same offence once per vehicle class in parallel
-- articles (Điều 6 ô tô, Điều 7 mô tô, Điều 8 xe máy chuyên dùng). The clause
-- bodies are near-identical, so every one of the 10 Hit@1 misses returned the
-- right document and the wrong vehicle.
--
-- The class is categorical, so it is applied as a multiplier on the fused score
-- rather than as a filter: a wrong classifier must cost ranking, never recall.
-- A chunk whose article names no vehicle (thẩm quyền, thủ tục, bảng tốc độ)
-- carries NULL and is left untouched.
-- ----------------------------------------------------------------------------

DROP FUNCTION IF EXISTS hybrid_search(TEXT, VECTOR(384), DATE, INT, INT);
