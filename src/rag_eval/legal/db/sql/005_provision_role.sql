-- ----------------------------------------------------------------------------
-- PROVISION-ROLE FACET
--
-- The held-out set exposed a confusion orthogonal to vehicle class: "xe máy
-- không nhường đường cho người đi bộ bị phạt bao nhiêu" returned Luật TTATGTĐB,
-- which imposes the duty, rather than NĐ 168, which prices breaking it. Both
-- speak to the same act; only one answers "how much". Definitions behave the
-- same way -- "tốc độ khai thác tối đa là gì" wants Điều Giải thích từ ngữ, not
-- the article that uses the term.
--
-- Same soft-multiplier mechanism as the vehicle facet, and applied on top of
-- it: a chunk that is both the wrong vehicle and the wrong kind of provision
-- earns both penalties.
-- ----------------------------------------------------------------------------

DROP FUNCTION IF EXISTS hybrid_search(TEXT, VECTOR(384), DATE, INT, INT, TEXT);
...
BEGIN
    ...
    SELECT
         ...
         * CASE
             ...
             WHEN c.metadata->>'vehicle_class' IS NULL THEN 1.0
             WHEN c.metadata->>'vehicle_class' = vehicle_class THEN class_bonus
             ELSE class_penalty
           END
         * CASE
