-- ----------------------------------------------------------------------------
-- SEARCH TAKES THE QUESTION'S WORDS, NOT A HASH OF THEM
--
-- `overlay_topic TEXT` becomes `overlay_tokens TEXT[]`, and the boost is
-- looked up by overlap rather than equality. See `016_overlay_overlap.sql`
-- for the paraphrases that forced this.
--
-- NULL still means the overlay contributes nothing, so every existing caller
-- is unaffected and the feature stays off until an experiment says otherwise.
-- ----------------------------------------------------------------------------
