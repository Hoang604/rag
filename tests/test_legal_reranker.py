...
def _reranker(scores: dict[str, float], blend: float = 1.0) -> CrossEncoderReranker:
    engine = CrossEncoderReranker(blend=blend)
    engine._model = StubModel(scores)  # noqa: SLF001
    return engine
...
async def test_reordering_by_cross_encoder_score() -> None:
    out = await _reranker(SCORES).rerank("câu hỏi", list(HITS))
...
async def test_top_k_truncates_after_reordering_not_before() -> None:
    ...
    out = await _reranker(SCORES).rerank("câu hỏi", list(HITS), top_k=1)
...
async def test_the_model_reads_the_contextualised_text() -> None:
    ...
    engine = _reranker(SCORES)
    await engine.rerank("câu hỏi", list(HITS))
    pairs = engine._model.calls[0]  # noqa: SLF001
...
async def test_nothing_to_reorder_costs_no_model_call(hits: list[SearchHit]) -> None:
    ...
    assert engine._model.calls == []  # noqa: SLF001
...
async def test_blending_lets_the_fused_order_still_count() -> None:
    ...
    pure = await _reranker(scores, blend=1.0).rerank("q", list(HITS))
    blended = await _reranker(scores, blend=0.5).rerank("q", list(HITS))
...
async def test_ties_do_not_lose_candidates() -> None:
    ...
    out = await _reranker(flat).rerank("câu hỏi", list(HITS))
    assert sorted(h.path for h in out) == ["a", "b", "c"]
